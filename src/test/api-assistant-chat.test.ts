...
describe('api/assistant/chat', () => {
  ...
  it('proxies to Anthropic and logs', async () => {
    ...
    expect((global.fetch as any).mock.calls[0][0]).toContain('api.anthropic.com')
  })
})
import { describe, it, expect, vi, beforeEach, afterEach } from 'vitest'

// Hoisted mocks: mock auth helper and supabase createClient before handler is imported
vi.mock('../../api/_lib/auth', () => ({
  getUserFromReq: async () => ({ user: { id: '1111-2222-3333-4444' } }),
}))
...
vi.mock('@supabase/supabase-js', () => ({
  createClient: () => ({
    from: () => ({
      ...
      const handlerModule = await import('../../api/assistant/chat.js')
      const req = makeReq({ authorization: 'Bearer token' }, {})
      const res = makeRes()
      // @ts-ignore
...
function makeRes() {
  const res: any = {}
      const handlerModule = await import('../../api/assistant/chat.js')
      const req = makeReq({ authorization: 'Bearer token' }, { messages: [{ role: 'user', content: 'Hi' }] })
      const res = makeRes()
      // @ts-ignore
      await handlerModule.default(req, res)
