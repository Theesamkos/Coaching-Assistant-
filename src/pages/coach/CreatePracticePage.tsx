...
import { Drill, EnhancedPlayer, Team, PracticeStatus } from '@/types'
import {
  ArrowLeftIcon,
  CalendarIcon,
  ClockIcon,
...
export default function CreatePracticePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Create practice
      const { data: practice, error: practiceError } = await practiceService.createPractice(
        userProfile.id,
        {
          title,
      ...
      if (playerIds.length > 0) {
        await practiceService.assignPlayersToPractice(practice.id, playerIds)
      }

      // Navigate to practice detail page
      navigate(`/coach/practices/${practice.id}`)
    } catch (err: any) {
  ...
  return (
    ...
    >
      ...
      <div className="mb-6">
        ...
        <h1 className="text-3xl font-bold text-white mb-2">Create Practice</h1>
        <p className="text-slate-400">Schedule a new practice session</p>
      </div>
          ...
          {playerAssignmentMode === 'individual' && (
            <div className="space-y-2 max-h-64 overflow-y-auto">
              ...
              {availablePlayers.length === 0 ? (
                <p className="text-slate-400 text-center py-8">
                  No players available. Invite players first!
                </p>
              ) : (
